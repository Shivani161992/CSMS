...
/// <summary>
/// Summary description for Export_wp
/// </summary>

[WebService(Namespace = "http://microsoft.co.in/", Name = "RunnerServiceWheatProcurement2013", Description = "Export Data (upload data on server)/Date: 30012013")]
[WebServiceBinding(ConformsTo = WsiProfiles.BasicProfile1_1)]


public class Export_wp : System.Web.Services.WebService
{
    ...
    public SecurityExport securityheadexport;
    [SoapHeader("securityheadexport")]
    [WebMethod]
    public bool chkSecurityExport(SecurityExport S)
    {
        bool rtev = false;
        if (S != null)
        {
    ...
    public bool InRunner(string SocietyId, string RunnerVer, string DistrictId)
    {
        ...
        try
        {
            ...
            commandt.CommandText = "Select count(*) from RunnerRegistration where  District_ID='" + DistrictId + "' and RunnerID='" + SocietyId + "'  ";
                ...
                if (dsTransport.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dsTransport.Tables[0].Rows)
                    {
                        ...
                        if (res <= 0)
                        {
                            ...
                            cmd.Transaction = trans;
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.CommandText = "in_TransportMaster";
                            cmd.Parameters.Clear();
                ...
                if (dsCommodityReceipt.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dsCommodityReceipt.Tables[0].Rows)
                    {
                        ...
                        if (res <= 0)
                        {
                            ...
                            cmd.Transaction = trans;
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.CommandText = "in_CommodityReceivedFromFarmer";
                            cmd.Parameters.Clear();
                ...
                if (dsIssueToSangrhan.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dsIssueToSangrhan.Tables[0].Rows)
                    {
                        ...
                        if (res <= 0)
                        {
                            ...
                            cmd.Transaction = trans;
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.CommandText = "in_IssueToSangrahan";
                            cmd.Parameters.Clear();
                ...
                if (dsGunnyBagsReceipt.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dsGunnyBagsReceipt.Tables[0].Rows)
                    {
                        ...
                        if (res <= 0)
                        {
                            ...
                            // cmd.CommandTimeout = 0;
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.CommandText = "in_GunnyBagsReceipt";
                            cmd.Parameters.Clear();
                ...
                if (dsGunnyBagsIssue.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dsGunnyBagsIssue.Tables[0].Rows)
                    {
                        ...
                        if (res <= 0)
                        {
                            ...
                            cmd.Transaction = trans;
                            cmd.CommandType = CommandType.StoredProcedure;
                            cmd.CommandText = "in_GunnyBagsIssue";
                            cmd.Parameters.Clear();
    ...
    public void Exception(String Message)
    {
        ...
        //excmd.Connection = connection;
        //excmd.CommandType = CommandType.StoredProcedure;
        //excmd.CommandText = "in_Exception";
        //excmd.Parameters.Clear();
        //excmd.Parameters.AddWithValue("@Message", Message);
        //excmd.Parameters.AddWithValue("@Date", System.DateTime.Now.ToString());
        //excmd.Parameters.AddWithValue("@PC_ID", "");
        //int x = excmd.ExecuteNonQuery();
        //CloseConnection();
...
public class SecurityExport : System.Web.Services.Protocols.SoapHeader
{
