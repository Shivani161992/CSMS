    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["dist_id"] != null)
        {
            if (!IsPostBack)
            {
                ...
                txt_FrmDist.Text = Session["dist_id"].ToString();
                GetMillName();
                //GetMPIssueCentre();
                GetOnlyDistName();
    ...
    public void GetOnlyDistName()
    {
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                select = "SELECT district_name,district_code FROM pds.districtsmp where district_code!='" + txt_FrmDist.Text + "' Order By district_name";
    ...
    public void GetMillName()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                select = "Select distinct PM.Mill_Name As MillCode,MR.Mill_Name As MillName From PaddyMilling_Agreement_2017 As PM Left Join Miller_Registration_2017 MR ON(PM.Mill_Name=MR.Registration_ID and PM.Mill_Addr_District=MR.District_Code and PM.CropYear=MR.CropYear) where PM.District='" + txt_FrmDist.Text + "' and PM.CropYear='" + txtYear.Text + "' and PM.IsAccepted='Y' order by MillName Asc";
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                if (ddlMillName.SelectedIndex > 0)
                {
                    ...
                    select = "Select Agreement_ID From PaddyMilling_Agreement_2017 where District='" + txt_FrmDist.Text + "' and Mill_Name='" + ddlMillName.SelectedValue.ToString() + "' and CropYear='" + txtYear.Text + "' and IsAccepted='Y' order by Agreement_ID";
    ...
    public void GetMPIssueCentre()
    {
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "select DepotName,DepotID from tbl_MetaData_DEPOT where DistrictId= '23" + txt_FrmDist.Text + "' order by DepotName";
                da = new SqlDataAdapter(select, con);
    ...
    public void GetBranch()
    {
        using (con_MPStorage = new SqlConnection(strcon_MPStorage))
        {
            try
            {
                ...
                {
                    string select1 = string.Format("select DepotName,BranchId from tbl_MetaData_DEPOT where DistrictId= '23" + txt_FrmDist.Text + "' order by DepotName");
    ...
    public void GETMappingGodown()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "";
                select = "Select Godown_id From PM_DistToGodown Where FrmDistrict='" + txt_FrmDist.Text + "' and ToDistrict='" + ddlFrmDist1.SelectedValue.ToString() + "' and CropYear='" + txtYear.Text + "' ";
    ...
    public void GetGodownBalance()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                cmdpqty.Parameters.Add("@district_code", SqlDbType.NVarChar).Value = txt_FrmDist.Text;
            ...
            else
            {
                ...
                if (count == 0)
                {
                    ...
                    if (checkDupGodown == 0)
                    {
                        ...
                        if (chkChange.Checked && txt_FrmDist.Text!="0")
                        {
                            dr["DistCode"] = ddlFrmDist1.SelectedValue.ToString();
                        }
                        else
                        {
                            dr["DistCode"] = txt_FrmDist.Text;
                        }
