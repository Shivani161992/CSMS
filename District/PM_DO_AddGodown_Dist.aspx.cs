...
public partial class District_PM_DO_AddGodown_Dist : System.Web.UI.Page
{
    ...
    public void GetDistName()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                if (Session["dist_id"].ToString() != "0")
                {
                    //select = "SELECT district_name,district_code FROM pds.districtsmp where paddy_markfed='Y' and district_code='" + DistCode + "' Order By district_name";
                ...
                if (ds != null)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        txt_FrmDist.Text = ds.Tables[0].Rows[0]["district_name"].ToString();
                    }
                }
            }
            catch (Exception ex)
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                if (ddlMillName.SelectedIndex > 0)
                {
                    ...
                    select = "Select Agreement_ID From PaddyMilling_Agreement_2017 where District='" + DistCode + "' and Mill_Name='" + ddlMillName.SelectedValue.ToString() + "' and CropYear='" + txtYear.Text + "' and IsAccepted='Y' order by Agreement_ID";
    ...
    public void GetMPIssueCentre()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "select DepotName,DepotID from tbl_MetaData_DEPOT where DistrictId= '23" + DistCode + "' order by DepotName";
                da = new SqlDataAdapter(select, con);
    ...
    public void GetBranch()
    {
        using (con_MPStorage = new SqlConnection(strcon_MPStorage))
        {
            try
            {
                ...
                {
                    string DistCode = Session["dist_id"].ToString();
                    string select1 = string.Format("select DepotName,BranchId from tbl_MetaData_DEPOT where DistrictId= '23" + DistCode + "' order by DepotName");
    ...
    public void GETMappingGodown()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "";
                select = "Select Godown_id From PM_DistToGodown Where FrmDistrict='" + DistCode + "' and ToDistrict='" + ddlFrmDist1.SelectedValue.ToString() + "' and CropYear='" + txtYear.Text + "' ";
            ...
            else
            {
                ...
                if (count == 0)
                {
                    ...
                    if (checkDupGodown == 0)
                    {
                        ...
                        if (chkChange.Checked && DistCode != "0")
                        {
                            dr["DistCode"] = ddlFrmDist1.SelectedValue.ToString();
                        }
                        else
                        {
                            dr["DistCode"] = DistCode;
                        }
