...
public partial class IssueCenter_Receipt_Entry_MO : System.Web.UI.Page
{
    ...
    public void GetDeliveryChallan()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "Select DC_MO,REPLACE(CONVERT(NVARCHAR,CreatedDate, 111), '/', '-') + ' ' + CONVERT(varchar(35),CreatedDate,114) As CreatedDate From DeliveryChallan_MO where ModeofDispatch='12' and ToDist='" + Dist_Id + "'  and (IsReceived='N' and ((DATEADD(DAY,9999,CreatedDate))>=Getdate())) and CreatedDate>='2015-10-1' and DC_MO NOT IN (Select challan_no From tbl_Receipt_Details where Dist_Id='" + Dist_Id + "' and S_of_arrival='02' and challan_no LIKE 'MORD%') and DC_MO NOT IN (Select DC_MO From DivertRecieved_MO where  DC_MO LIKE 'MORD%') order by DC_MO";
    ...
    public void GetMPIssueCentre()
    {
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = string.Format("select DepotName,DepotID from tbl_MetaData_DEPOT where DistrictId= '23" + ddldistrict.SelectedValue + "' order by DepotName");
                da = new SqlDataAdapter(select, con);
    ...
    public void GenrateNewMO()
    {
        Random RND = new Random();
        TrNMN.Visible = true;
        DivMovmentNumber = Int64.Parse(txtMONo.Text + "" + RND.Next(0, 999999));

        lblMovmentNumber.Text = Convert.ToString(DivMovmentNumber);
    ...
    public void GetPageNoDataFrmDC()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "Select (select DepotName from tbl_MetaData_DEPOT where DepotID=DC.RecIC) RecICName,DC.CropYear,(select district_name From pds.districtsmp where district_code= DC.FrmDist) FrmDistName,DC.FrmDist,DC.ToDist,(Select Commodity_Name from tbl_MetaData_STORAGE_COMMODITY where Commodity_Id=DC.Commodity) ComdtyName,DC.Commodity,DC.Issued_Date,DC.Issued_Bags,DC.Issued_Qty,DC.Truck_No,DC.Bags_Type,(select Top 1 (Transporter_Name) from Transporter_Table where Transporter_ID =DC.Transporter_ID and Distt_ID=DC.FrmDist and Transport_ID in ('10','11','6','1') order by Transporter_Name desc) TransporterName,DC.Transporter_ID,DC.DC_No,DC.STO_No,DC.TO_No,DC.SMO,DC.MoveOrdernum,Dc.ModeofDispatch,Dc.TOEndDate,DC.RecGodown,DC.RecBranch,DC.RecIC From DeliveryChallan_MO DC where DC.FrmDist='" + ddlChallanNo.SelectedValue.ToString() + "' and DC.ToDist='" + Dist_Id + "' and RecIC='" + IC_Id + "' and DC.ModeofDispatch='13' and DC.IsReceived='N' and DC.Issue_Center='0000' and DC.Default_Branch='0000' and DC.Book_No='" + ddlBookNo.SelectedItem.ToString() + "' and DC.Page_No='" + ddlPageNo.SelectedItem.ToString() + "' and DC.DC_MO='" + ddlPageNo.SelectedValue.ToString() + "' ";
                da = new SqlDataAdapter(select, con);
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    txtBagsType.Text = ds.Tables[0].Rows[0]["Bags_Type"].ToString();
    ...
    public void GetChallanData()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "Select RecGodown,RecBranch,(select DepotName from tbl_MetaData_DEPOT where DepotID=DCMO.RecIC) RecICName,RecIC,Truck_No,Bags_Type,Issued_Bags,Issued_Date,Issued_Qty,Change_Godown,Change_Branch,Default_Godown,Default_Branch,Issue_Center,ModeofDispatch,TOEndDate,CropYear,(Select Commodity_Name from tbl_MetaData_STORAGE_COMMODITY where Commodity_Id=DCMO.Commodity) ComdtyName,Commodity,ToDist,MoveOrdernum,SMO,TO_No,STO_No,DC_No,(select Top 1 (Transporter_Name) from Transporter_Table where Transporter_ID =DCMO.Transporter_ID and Distt_ID=DCMO.FrmDist and Transport_ID in ('10','11','6') order by Transporter_Name desc) TransporterName,Transporter_ID,(select district_name From pds.districtsmp where district_code= DCMO.FrmDist) FrmDistName,FrmDist From DeliveryChallan_MO DCMO where DC_MO='" + ddlChallanNo.SelectedItem.ToString() + "' and ToDist='" + Dist_Id + "' and ModeofDispatch='12' and IsReceived='N' and CreatedDate='" + ddlChallanNo.SelectedValue.ToString() + "'";
                //string select = "Select RecGodown,RecBranch,RecIC,Truck_No,Bags_Type,Issued_Bags,Issued_Date,Issued_Qty,Change_Godown,Change_Branch,Default_Godown,Default_Branch,Issue_Center,ModeofDispatch,TOEndDate,CropYear,(Select Commodity_Name from tbl_MetaData_STORAGE_COMMODITY where Commodity_Id=DCMO.Commodity) ComdtyName,Commodity,ToDist,MoveOrdernum,SMO,TO_No,STO_No,DC_No,(select Top 1 (Transporter_Name) from Transporter_Table where Transporter_ID =DCMO.Transporter_ID and Distt_ID=DCMO.FrmDist and Transport_ID in ('10','11','6') order by Transporter_Name desc) TransporterName,Transporter_ID,(select district_name From pds.districtsmp where district_code= DCMO.FrmDist) FrmDistName,FrmDist From DeliveryChallan_MO DCMO where DC_MO='" + ddlChallanNo.SelectedItem.ToString() + "' and ToDist='" + Dist_Id + "' and ModeofDispatch='12' and IsReceived='N'  and CreatedDate='" + ddlChallanNo.SelectedValue.ToString() + "'";
                ...
                if (Session["update"].ToString() == ViewState["update"].ToString())
                {
                    using (con = new SqlConnection(strcon))
                    {
                        try
                        {
                            ...
                            //string IssuedDate = ServerDate.getDate_MDY(DateTime.Now.ToString());
                            ...
                            {
                                IC_Id = Session["dist_id"].ToString();
                            }
                            Dist_Id = Session["dist_id"].ToString();
                            string opid = Session["dist_id"].ToString();
                            ...
                            if (ViewState["hdfModeofDispatch"].ToString() == "12") //Received By Road
                            {
                                ...
                                ReceiptID = "";
                                getnum = 0;
                                String SelectReceiptID = "select max(Receipt_id) as Receipt_id from dbo.tbl_Receipt_Details where  Dist_Id='" + Dist_Id + "'";
                                ds1 = new DataSet();
                                ...
                                else
                                {
                                    ...
                                    //instr += "Insert Into [DivertRecieved_MO]([MoveOrdernum],[DivMoveOrdernum],[DC_No],[DC_MO],[ReceiptID],[Div_RecDistrict],[Div_RecBranch],[Div_RecGodown],[Div_RecIssue_Center],[Div_RecDate],[IP]) values('" + txtMONo.Text + "','" + lblMovmentNumber.Text + "', '" + ViewState["hdfDC_No"].ToString() + "','" + ddlChallanNo.SelectedItem.ToString() + "','" + ReceivedRecptID + "','" + ddldistrict.SelectedItem.Text + "','" + ddlBranch.SelectedItem.Text + "','" + ddlGodown.SelectedItem.Text + "','" + ddlIssueCentre.SelectedItem.Text + "','" + IssuedDate + "','" + GetIp + "')";
                                    instr += "Insert Into [DivertRecieved_MO]([MoveOrdernum],[DivMoveOrdernum],[SMO],[TO_No],[DC_No],[Transporter_ID],[FrmDist],[ToDist],[Commodity],[CropYear],[TOEndDate],[ModeofDispatch],[DC_MO],[ReceiptID],[SendingIssue_Center],[SendingBranch],[SendingGodown],[SendingQty],[SendingBags],[SendingDate],[SendingBags_Type],[SendingTruck_No],[Div_RecDistrict],[Div_RecBranch],[Div_RecGodown],[Div_RecIssue_Center],[Div_RecDate],[IP]) values('" + txtMONo.Text + "','" + lblMovmentNumber.Text + "','" + ViewState["hdfSMO"].ToString() + "','" + txtTONo.Text + "','" + ViewState["hdfDC_No"].ToString() + "','" + ViewState["hdfTransporter_ID"].ToString() + "','" + ViewState["hdfFrmDist"].ToString() + "','" + Dist_Id + "','" + ViewState["hdfCommodity"].ToString() + "','" + txtCropYear.Text + "','" + ViewState["hdfTOEndDate"].ToString() + "','" + ViewState["hdfModeofDispatch"].ToString() + "','" + ddlChallanNo.SelectedItem.ToString() + "','" + ReceivedRecptID + "','" + ViewState["hdfSendIssue_Center"].ToString() + "','" + strSendChange_Branch + "','" + strSendChange_Godown + "','" + SendQty + "','" + txtSendBags.Text + "','" + ViewState["hdfTranspDate"].ToString() + "','" + txtBagsType.Text + "','" + txtTruckNo.Text + "','" + ddldistrict.SelectedItem.Text + "','" + ddlBranch.SelectedItem.Text + "','" + ddlGodown.SelectedItem.Text + "','" + ddlIssueCentre.SelectedItem.Text + "','" + IssuedDate + "','" + GetIp + "')";
                                }
                            }
