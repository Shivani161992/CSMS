...
public partial class IssueCenter_CSC_ChanaMasoor : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd, cmd1;
    ...
    private void GetWeighbridgeName()
    {
        using (con = new SqlConnection(Con_CSMS))
        {
            try
            {
                ...
                select = "SELECT   [Weigh_Id] ,[WighBridge_Number] FROM [WeighBridge_Master] where District_Code = '" + Dist_Id + "' and IssueCenter = '" + IC_Id + "'";
                            ...
                            if (dsP.Tables[0].Rows.Count == 0)
                            {
                                ...
                                if (Convert.ToInt16(str1) == 0)   // not Found, Insert start
                                {
                                    try
                                    {
                                        ...
                                        cmdacno.CommandText = "prc_getMAxAcceptance_NoCSM2018";
