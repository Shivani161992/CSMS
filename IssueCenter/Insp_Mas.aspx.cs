...
public partial class IssueCenter_Insp_Mas : System.Web.UI.Page
{
            ...
            try
            {
                ...
                if (ds != null)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (Session["dist_id"] != null)
                        {
                            Session["district_code"].ToString();
                        }
                    }
                }
            }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        using(con = new SqlConnection(strcon))
            ...
            {
                con.Open();
                string qrey = "select isnull(max(Inspector_ID),0) as Inspector_ID  from Inspector_Master_02017 where  district='" + district_code + "' and LEN(Inspector_ID)<8 ";
                da = new SqlDataAdapter(qrey, con);

                ds = new DataSet();
                da.Fill(ds);
                //mobj1 = new MoveChallan(ComObj);
                //string qrey = "select isnull(max(Inspector_ID),0) as Transporter_ID  from Inspector_Master_02017 where  Distt_ID='" + district_code + "' and LEN(Inspector_ID)<8 ";
                //DataSet ds = new DataSet();
                // dmax.Fill(ds);
