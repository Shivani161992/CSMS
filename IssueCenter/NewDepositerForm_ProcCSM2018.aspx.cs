...
public partial class IssueCenter_NewDepositerForm_ProcCSM2018 : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd, cmd1;
    ...
    private void GetAcDate()
    {
        ...
        using (con = new SqlConnection(Con_CSMS))
        {
            try
            {
                ...
                string select = "";
                select = "SELECT distinct convert(varchar(10),Acceptance_Date,105)Acceptance_Date FROM SCSC_Procurement_CSM where AN_Status = 'Y' and Distt_ID = '" + Dist_Id + "' and IssueCenter_ID = '" + IC_Id + "' and SCSC_Procurement_CSM.TC_Number not in (select TC_Number from DepositerForm_CSM2018 where DepositerForm_CSM2018.IssueCenter_ID = '" + IC_Id + "')";
                con.Open();
                da = new SqlDataAdapter(select, con);
                ds = new DataSet();
                da.Fill(ds);
        ...
        using (con = new SqlConnection(Con_CSMS))
        {
            try
            {
                ...
                if (ddlcommodtiy.SelectedValue.ToString() == "33" || ddlcommodtiy.SelectedValue.ToString() == "63" || ddlcommodtiy.SelectedValue.ToString() == "64")
                {
                    ...
                    qrydata = "SELECT Society_MSP.Society_Id , Society_MSP.Society_Name + ' ( '+ Society_MSP.SocPlace + ' )' as Society ,SCSC_Procurement_CSM.TC_Number , SCSC_Procurement_CSM.NetWeight , SCSC_Procurement_CSM.Recd_Bags ,SCSC_Procurement_CSM.Acceptance_No, SCSC_Procurement_CSM.Receipt_Id , SCSC_Procurement_CSM.Recd_Godown , tbl_MetaData_GODOWN.Godown_Name  FROM SCSC_Procurement_CSM inner join Society_MSP on Society_MSP.Society_Id = SCSC_Procurement_CSM.Purchase_Center  inner join tbl_MetaData_GODOWN on tbl_MetaData_GODOWn.Godown_ID = SCSC_Procurement_CSM.Recd_Godown  where  convert(varchar(10),Acceptance_Date,101) = '" + pdate + "' and SCSC_Procurement_CSM.IssueCenter_ID = '" + IC_Id + "' and SCSC_Procurement_CSM.TC_Number not in (select TC_Number from DepositerForm_CSM2018 where DepositerForm_CSM2018.IssueCenter_ID = '" + IC_Id + "' and DepositerForm_CSM2018.CommodityId = '"+ddlcommodtiy.SelectedValue.ToString()+"') and SCSC_Procurement_CSM.Commodity_Id = '"+ddlcommodtiy.SelectedValue.ToString()+"' ";
