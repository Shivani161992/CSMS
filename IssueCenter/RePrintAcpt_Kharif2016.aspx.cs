...
public partial class IssueCenter_RePrintAcpt_Kharif2016 : System.Web.UI.Page
{
    SqlConnection con;
    SqlDataAdapter da;
    DataSet ds;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["issue_id"] != null)
        {
            if (!IsPostBack)
            {
                ...
                Session["Acceptance_NO"] = Session["Godown"] = null;
                GetCommodity();
            }
        }
        else
    ...
    private void GetReceiptID()
    {
        using (con = new SqlConnection(Con_CSMS))
        {
            try
            {
                ...
                select = "Select distinct Acceptance_No From Acceptance_Note_Kharif2016 Where Distt_ID='" + Dist_Id + "' and IssueCenter_ID='" + ICID + "' and CommodityId='" + ddlCommodity.SelectedValue.ToString() + "'and Acceptance_Date between '" + ConvertFromDate + " 00:00:00' and '" + ConvertToDate + " 23:59:59' and godown='" + ddlGodown.SelectedValue.ToString() + "' and Accept_Qty!='0'";
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    ddlReceiptID.DataTextField = "Acceptance_No";
                    ddlReceiptID.DataValueField = "Acceptance_No";
                    ddlReceiptID.DataBind();
                    ddlReceiptID.Items.Insert(0, "--Select--");
                }
                else
                {
                    Page.RegisterClientScriptBlock("mymsg1", "<script language=javascript> alert('Acceptance No. Is Not Available On Selected Date'); </script> ");
                    return;
                }
            }
            catch (Exception ex)
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        ...
        {
            Session["Acceptance_NO"] = ddlReceiptID.SelectedItem.ToString();
        ...
        {
            Page.RegisterClientScriptBlock("mymsg1", "<script language=javascript> alert('Please Select Acceptance No.'); </script> ");
