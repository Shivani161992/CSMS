...
public partial class IssueCenter_RePrintReceipt_Kharif2016 : System.Web.UI.Page
{
    SqlConnection con;
    SqlDataAdapter da;
    DataSet ds;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Session["issue_id"] != null)
        {
            if (!IsPostBack)
            {
                ...
                Session["Receipt_ID"] = Session["Commodity_ID"] = null;
                GetCommodity();
            }
        }
        else
    ...
    private void GetReceiptID()
    {
        using (con = new SqlConnection(Con_CSMS))
        {
            try
            {
                ...
                select = "Select Receipt_Id From SCSC_Procurement_Kharif2016 Where Distt_ID='" + Dist_Id + "' and IssueCenter_ID='" + ICID + "' and Commodity_Id='" + ddlCommodity.SelectedValue.ToString() + "'and Recd_Date between '" + ConvertFromDate + " 00:00:00' and '" + ConvertToDate + " 23:59:59' and Recd_Godown='" + ddlGodown.SelectedValue.ToString() + "' and Book_No='0'";
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    ddlReceiptID.DataTextField = "Receipt_Id";
                    ddlReceiptID.DataValueField = "Receipt_Id";
                    ddlReceiptID.DataBind();
                    ddlReceiptID.Items.Insert(0, "--Select--");
                }
                else
                {
                    Page.RegisterClientScriptBlock("mymsg1", "<script language=javascript> alert('Receipt ID Is Not Available On Selected Date'); </script> ");
                    return;
                }
            }
            catch (Exception ex)
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        ...
        {
            Session["Receipt_ID"] = ddlReceiptID.SelectedItem.ToString();

            if (ddlCommodity.SelectedValue.ToString() == "13")
            {
                Session["Commodity_ID"] = "2";
            }
            else if (ddlCommodity.SelectedValue.ToString() == "14")
            {
                Session["Commodity_ID"] = "3";
            }
            else if (ddlCommodity.SelectedValue.ToString() == "8")
            {
                Session["Commodity_ID"] = "6";
            }
            else if (ddlCommodity.SelectedValue.ToString() == "11")
            {
                Session["Commodity_ID"] = "4";
            }
            else if (ddlCommodity.SelectedValue.ToString() == "12")
            {
                Session["Commodity_ID"] = "5";
            }
            else if (ddlCommodity.SelectedValue.ToString() == "40")
            {
        ...
        {
            Page.RegisterClientScriptBlock("mymsg1", "<script language=javascript> alert('Please Select Receipt ID'); </script> ");
