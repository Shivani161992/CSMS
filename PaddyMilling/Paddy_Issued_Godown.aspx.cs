...
public partial class PaddyMilling_Paddy_Issued_Godown : System.Web.UI.Page
{
    ...
    public void GetMPIssueCentre()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                con.Open();
                string select = "";
                hdfDistanceDist.Value = hdfODDist.Value = "";
                ...
                select = "Select distinct Dist.IssueCenter As DepotID,PM.ToDistrict From Distance_Master_Godown As Dist Left Join PM_DistToGodown As PM ON(PM.FrmDistrict='" + districtid + "' and  PM.CropYear='" + txtYear.Text + "' and PM.ToDistrict=Dist.DistrictId and PM.Godown_id=Dist.Godown_id) where (Dist.DistrictId='" + districtid + "' OR PM.FrmDistrict='" + districtid + "' ) and Dist.PCCodeOrRailheadcode='" + ddlMillName.SelectedValue.ToString() + "' and Dist.Distance_For='11' Order by PM.ToDistrict Desc ";
                da = new SqlDataAdapter(select, con);
                ...
                {
                    hdfODDist.Value = ds.Tables[0].Rows[0]["ToDistrict"].ToString();
    ...
    public void GetWareHouseIC()
    {
        ...
        using (con_MPStorage = new SqlConnection(strcon_MPStorage))
        {
            try
            {
                ...
                string select = "select DepotName,DepotID from tbl_MetaData_DEPOT where (DistrictId= '23" + districtid + "' OR DistrictId= '23" + hdfODDist.Value + "') and StateId='23' and DepotID IN(" + hdfDistanceDist.Value + ") order by DepotName";
    ...
    public void GetGridDataForDistance()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "Select Dist.DistrictId,Dist.IssueCenter As ICName,Dist.IssueCenter,Dist.Godown_id,Dist.distance From Distance_Master_Godown As Dist Left Join PM_DistToGodown As PM ON(PM.FrmDistrict='" + districtid + "' and  PM.CropYear='" + txtYear.Text + "' and PM.ToDistrict=Dist.DistrictId and PM.Godown_id=Dist.Godown_id) where (Dist.DistrictId='" + districtid + "' OR PM.FrmDistrict='" + districtid + "' ) and Dist.PCCodeOrRailheadcode='" + ddlMillName.SelectedValue.ToString() + "' and Dist.Distance_For='11' --order by  CONVERT(float, Replace(Dist.distance, 'Group','')) Asc";
    ...
    protected void GridView2_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            using (con_MPStorage = new SqlConnection(strcon_MPStorage))
            {
                ...
                {
                    con_MPStorage.Open();
                    string qry = "select (select DepotName from tbl_MetaData_DEPOT where DepotId='" + ICName + "') BranchName,(select Godown_Name from tbl_MetaData_GODOWN where Remarks='Y' and Godown_ID='" + GodownName + "') Godown_Name";
                    string select = string.Format(qry);
                    da = new SqlDataAdapter(select, con_MPStorage);
    ...
    {
        string IC_Id = "", strGodownVal = "", DistCode = "";
        hdfAllGridGodown.Value = "";
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                {
                    hdfAllGridGodown.Value += ((hdfAllGridGodown.Value == "") ? "" : " , ") + "'" + GridView2.Rows[i].Cells[8].Text + "'";

                    IC_Id = GridView2.Rows[i].Cells[7].Text;
                    strGodownVal = GridView2.Rows[i].Cells[8].Text;
                    DistCode = GridView2.Rows[i].Cells[9].Text;

                    string select = "";

                    select = "Select Sum(Alloted_CommonPaddy) As Alloted_CommonPaddy From PaddyMapping_Godown where District='" + DistCode + "' and CropYear='" + txtYear.Text + "' and Godown_id='" + strGodownVal + "'";
    ...
    {
        string IC_Id = "", strGodownVal = "", DistID = "";
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                for (int i = 0; i < GridView2.Rows.Count; i++)
                {
                    IC_Id = GridView2.Rows[i].Cells[7].Text;
                    strGodownVal = GridView2.Rows[i].Cells[8].Text;
                    DistID = GridView2.Rows[i].Cells[9].Text;
                    ...
                    cmdpqty.Parameters.Add("@Openingdate", SqlDbType.NVarChar).Value = "01/04/2016";
    ...
    {
        districtid = Session["dist_id"].ToString();
        string TotalDhan, TotalGDHan;
        string TotalDhanIssue, TotalGDHanIssue;
        decimal DifferenceDhan, DifferenceDhanG;
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = string.Format("Select Common_Dhan,GradeA_Dhan From PaddyMilling_Agreement_2017 where District='" + districtid + "' and Mill_Name='" + ddlMillName.SelectedValue.ToString() + "' and CropYear='" + txtYear.Text + "' and Agreement_ID='" + ddlAgtmtNumber.SelectedItem.ToString() + "' and IsAccepted='Y' ");
                da = new SqlDataAdapter(select, con);
                ds = new DataSet();
                da.Fill(ds);
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    TotalDhan = ds.Tables[0].Rows[0]["Common_Dhan"].ToString();
                    TotalGDHan = ds.Tables[0].Rows[0]["GradeA_Dhan"].ToString();

                    string qry = string.Format("select isnull(sum(isnull(Alloted_CommonDhan,0)),0) as  Alloted_CommonDhan, isnull(sum(isnull(Alloted_GradeADhan,0)),0) as Alloted_GradeADhan  from PaddyMilling_DO_2017 where CropYear='" + txtYear.Text + "' and Mill_Code='" + ddlMillName.SelectedValue.ToString() + "' and Agreement_ID='" + ddlAgtmtNumber.SelectedValue.ToString() + "' and IsSociety='Y' ");
                    da = new SqlDataAdapter(qry, con);
                    ds = new DataSet();
                    da.Fill(ds);
                    if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                        TotalDhanIssue = ds.Tables[0].Rows[0]["Alloted_CommonDhan"].ToString();
                        TotalGDHanIssue = ds.Tables[0].Rows[0]["Alloted_GradeADhan"].ToString();
                        DifferenceDhan = Convert.ToDecimal(TotalDhan) - Convert.ToDecimal(TotalDhanIssue);
                        DifferenceDhanG = Convert.ToDecimal(TotalGDHan) - Convert.ToDecimal(TotalGDHanIssue);
                        txtTotalCDhan.Text = txtRemCommonDhan.Text = Convert.ToString(DifferenceDhan);
                        txtTotalGDhan.Text = txtRemGradeADhan.Text = Convert.ToString(DifferenceDhanG); 
                    }
        ...
        else
        {
            ...
            if (count == 0)
            {
                ...
                for (int i = 0; i < GridView2.Rows.Count; i++)
                {
                    string GridGodown = "";
                    dr["DistID"] = "";
                    GridGodown = GridView2.Rows[i].Cells[8].Text;

                    if (GridGodown == ddlGodown.SelectedValue.ToString())
                    {
                        dr["DistID"] = GridView2.Rows[i].Cells[9].Text;
                        break;
                    }
                }
    ...
    public void GetDistanceGodown()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                string select = "Select Godown_id From Distance_Master_Godown Where (DistrictId='" + districtid + "' OR DistrictId='" + hdfODDist.Value + "') and PCCodeOrRailheadcode='" + ddlMillName.SelectedValue.ToString() + "' and IssueCenter='" + ddlIssueCentre.SelectedValue.ToString() + "' and Distance_For='11' and Godown_id IN(" + hdfAllGridGodown.Value + ") --order by  CONVERT(float, Replace(distance, 'Group','')) Asc";
                da = new SqlDataAdapter(select, con);
                ...
                {
                    Page.RegisterClientScriptBlock("mymsg1", "<script language=javascript> alert('कृपया मिल से गोदाम की दुरी की मैपिंग करें|'); </script> ");
                }
            }
            catch (Exception ex)
            {
    ...
    public void GetGodown()
    {
        ...
        using (con_MPStorage = new SqlConnection(strcon_MPStorage))
        {
            try
            {
                ...
                string select = "select distinct Godown_ID,Godown_Name from tbl_MetaData_GODOWN where Remarks='Y' and (DistrictId='23" + districtid + "' OR DistrictId='23" + hdfODDist.Value + "') and Godown_ID IN (" + hdfGodown.Value + ") Order By Godown_Name";
                da = new SqlDataAdapter(select, con_MPStorage);
                ...
                if (Session["update"].ToString() == ViewState["update"].ToString())
                {
                    using (con = new SqlConnection(strcon))
                    {
                        try
                        {
                            ...
                            string select = "Select * From PaddyMapping_Godown where Mapping_No Like '" + ddlAgtmtNumber.SelectedValue.ToString() + "%' ";
                            da = new SqlDataAdapter(select, con);
                            ds = new DataSet();
                            da.Fill(ds);
                            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                            {
                                string LikeMappingNO = ddlAgtmtNumber.SelectedValue.ToString();
                                AgrmtNumber = LikeMappingNO.Substring(2);
                            }
