...
public partial class PaddyMilling_Print_PrintCMRAcceptance : System.Web.UI.Page
{
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    lblAcptCommon.Text = ds.Tables[0].Rows[0]["Accept_CommonRice"].ToString();
                    lblAcptGA.Text = ds.Tables[0].Rows[0]["Accept_GradeARice"].ToString();
                    ...
                    string selectData = string.Format("Select Mill_Addr_District,State,State_Code From PaddyMilling_Agreement_2017 where Mill_Name='" + hdfMillCode.Value + "' and Agreement_ID='" + hdfAgrmtID.Value + "'");
    ...
    {
        if (Session["MultiView"] == "CMR-Testing")
        {
            Response.Redirect("~/IssueCenter/Receipt_Entry_CMR.aspx");
        }
    }
}