...
public partial class PaddyMilling_Reprint_PMDO : System.Web.UI.Page
{
    SqlConnection con;
    SqlDataAdapter da;
    DataSet ds;
    ...
    public void GetDONumber()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                if (Session["DistrictManager"].ToString() == "DM" || Session["DistrictManager"].ToString() == "OP")
                {
                    select = "Select distinct Check_DO From PaddyMilling_DO_2017 where Mill_Code='" + ddlMillName.SelectedValue.ToString() + "' and Agreement_ID='" + ddlAgrmtNo.SelectedItem.ToString() + "' and District='" + DistCode + "' and CropYear='" + hdfCropYear.Value + "' and User_Agent!='DDMO' order by Check_DO";
                }
                else if (Session["DistrictManager"].ToString() == "DDMO")
                {
                    select = "Select distinct Check_DO From PaddyMilling_DO_2017 where Mill_Code='" + ddlMillName.SelectedValue.ToString() + "' and Agreement_ID='" + ddlAgrmtNo.SelectedItem.ToString() + "' and District='" + DistCode + "' and CropYear='" + hdfCropYear.Value + "' and User_Agent='DDMO' order by Check_DO";
                ...
                if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    ddlMvmtNumber.DataTextField = "Check_DO";
                    ddlMvmtNumber.DataValueField = "Check_DO";
                ...
                {
                    Page.RegisterClientScriptBlock("mymsg1", "<script language=javascript> alert('DO Number Is Not Available'); </script> ");
