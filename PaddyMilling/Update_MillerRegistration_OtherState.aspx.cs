...
public partial class PaddyMilling_Update_MillerRegistration_OtherState : System.Web.UI.Page
{
       ...
       // string DistCode = Session["dist_id"].ToString();
        ...
        using (con = new SqlConnection(strcon))
        {
            try
            {
                ...
                select = "select mill_name, Registration_ID from Miller_Registration_2017 where State_Code='"+ddlOtherState.SelectedValue.ToString()+"' and Status='0' and CropYear='" + ddlCropyear.SelectedValue.ToString() + "'order by mill_name Asc";
                da = new SqlDataAdapter(select, con);
                ds = new DataSet();
                da.Fill(ds);
    ...
    public void GetMillData()
    {
        ...
        using (con = new SqlConnection(strcon))
        {
            ...
            {
                con.Open();
                string select = string.Format("select milling_capacity_arwa, milling_capacity_usna, upcoming_SixMonths, VacantCap, TotalVacantCap from Miller_Registration_2017 where State_Code='"+ddlOtherState.SelectedValue.ToString()+"' and CropYear='" + ddlCropyear.SelectedValue.ToString() + "' and Status='0' and Registration_ID='" + ddlMillName.SelectedValue.ToString() + "'");
                da = new SqlDataAdapter(select, con);
