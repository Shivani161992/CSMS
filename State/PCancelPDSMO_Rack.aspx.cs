...
public partial class State_PCancelPDSMO_Rack : System.Web.UI.Page
{
    SqlConnection con;
    SqlCommand cmd;
    SqlDataAdapter da;
    ...
    public void GetData()
    {
        using (con = new SqlConnection(strcon))
        {
            ...
            {
                con.Open();
                string select = string.Format("SELECT ModeofDist,(Select Commodity_Name from tbl_MetaData_STORAGE_COMMODITY where Commodity_Id=Commodity) ComdtyName,Commodity,ReachDate,(select Source_Name from  Source_Arrival_Type where Source_ID=ModeofDispatch) DispatchModeName,ModeofDispatch,(SELECT district_name FROM pds.districtsmp where district_code=FrmDist) FromDistName,FrmDist,CropYear,GETDATE() CurrentDate,CreatedDate,GunnyType,CancelledDate,Cancelled_EmpID,Cancelled_MobileNo FROM StateMovementOrder where MoveOrdernum='" + hdfMovmtOrderNo.Value + "' and IsCancelled='Y'");
                ...
                if (ds != null)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (ModeofDist == "Other")
                        {
                            lblDistMode.Text = "रैक प्राप्तकर्ता जिले द्वारा स्कंध का परिवहन अन्य जिलों में निम्नानुसार रोक दिया जाये :";
                        }
                        else
                        {
                            lblDistMode.Text = "रैक प्राप्तकर्ता जिले द्वारा स्कंध का परिवहन स्वयं तथा अन्य के जिलों में निम्नानुसार रोक दिया जाये :";
                        }
                    ...
                    select = string.Format("Select Quantity,(SELECT district_name FROM pds.districtsmp where district_code=ToDist) ReceiveDistName,ToDist From StateMovementOrder where MoveOrdernum='" + hdfMovmtOrderNo.Value + "'  and IsCancelled='Y'");
                    da = new SqlDataAdapter(select, con);
                    ds = new DataSet();
                    da.Fill(ds);
